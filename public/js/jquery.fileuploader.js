				...
				f = {
					...
					thumbnails: {
						...
						item: function(item) {
							...
							item.renderImage = function(src) { f.thumbnails.renderThumbnail(item, true, src); };
						 ...
						 * @param {string} custom image source
                         * @void
                         */
						renderThumbnail: function(item, forceRender, src) {
							...
							if (['image', 'video', 'audio'].indexOf(item.format) > -1 && f.isFileReaderSupported() && (item.appended || n.thumbnails.startImageRenderer || forceRender)) {
                                ...
                                if (src)
                                    load(src);
                                else if (typeof item.file == "string" && false)
                                    load(item.file);
                                else
	...
	$.fn.fileuploader.defaults = {
        ...
        thumbnails: {
            ...
            item2: '<li class="fileuploader-item">' +
                    ...
                    '</li>',
            popup: {
                template: '<div class="fileuploader-popup">' +
                    '<div class="fileuploader-popup-preview">${reader.node}</div>' +
                    '<div class="fileuploader-popup-tools">' +
                        '<div class="left-side">' +
                            '<p>Name:</p>' +
                            '<h3>${name}</h3>' +
                        '</div>' +
                        '<div class="right-side">' +
                            '<a class="fileuploader-popup-button fileuploader-popup-remove">${captions.remove}</a>' +
                            '<a class="fileuploader-popup-button button-success fileuploader-popup-close">${captions.close}</a>' +
                        '</div>' +
                    '</div>' +
                '</div>',
